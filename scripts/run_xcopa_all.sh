...
do
    for lang in vi zh # et id it sw th tr 
    do
        k=8
...
#         k=8
#         echo "Running for language $lang and prompt ${prompt_name} and k $k"
#         python -m mega.eval_xcopa -p $lang -t $lang --pivot_prompt_name "${prompt_name}" --tgt_prompt_name "${prompt_name}" -k $k --model "gemini-pro" -d xcopa --timeout 30 --eval_on_val
#     done
# done