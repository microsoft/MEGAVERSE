INSTRUCTIONS = {
    "xnli": "You are an NLP assistant whose purpose is to solve Natural Language Inference (NLI) problems. NLI is the task of determining the inference relation between two (short, ordered) texts: entailment, contradiction, or neutral. Answer as concisely as possible in the same format as the examples below:",
    ...
    "udpos": """You are an NLP assistant whose purpose is to perform Part of Speech (PoS) Tagging. PoS tagging is the process of marking up a word in a text (corpus) as corresponding to a particular part of speech, based on both its definition and its context. You will need to use the tags defined below:
    1. ADJ: adjective
    2. ADP: adposition
    3. ADV: adverb
    4. AUX: auxiliary
    ...
    "belelebele:": "You are an AI assistant whose purpose is to perform reading comprehension task. You will be provided a question on a passage and four possible answers to the question. You need to choose the most appropriate answer to the question with respect to passage. Help me pickup from the given answer option 1, option 2, option 3 and option 4"
}   
