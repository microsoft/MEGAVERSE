    ...
    do
        ...
        for lang in "en" "de" "es" "fr" "ja" "ko" "zh"
            do
                ...
                python -m mega.hf_models.src.eval_pawsx -d "paws-x" -p "$lang" -t "$lang" --pivot_prompt_name "$prompt_name" --tgt_prompt_name "$prompt_name" -k "$k" --model "${model}" -e melange --chat-prompt
            done
    done