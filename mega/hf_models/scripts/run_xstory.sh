for prompt_name in "Answer Given options"
    ...
    for model in "meta-llama/Llama-2-7b-chat-hf" "meta-llama/Llama-2-13b-chat-hf"
        ...
        do
            ...
            python -m mega.hf_models.src.eval_xstory_cloze -d xstory_cloze -e melange -p en -t $lang --model "${model}" --tgt_prompt_name "${prompt_name}" --temperature 0 --log_wandb -k $k --timeout 30 --chat-prompt
        done
    done
done 
