...
do 
    ...
    do
        echo "Running for language $lang"
        python -m mega.eval_afriqa_gptturbo -p $lang -t $lang --pivot_prompt_name "answer_given_context_and_question" --tgt_prompt_name "answer_given_context_and_question" -k 8 --model $model -e melange --chat_prompt --temperature 0 --num_evals_per_sec 2 -d afriqa --test_frac 0.15 --from_hf_hub --save_dir $save_dir $save_dir
    done
done